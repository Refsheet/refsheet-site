...
const CharacterSaleButton = ({ character, className }) => {
  ...
  return (<Button className={c(className, 'btn-secondary character-sale-button')}>
    <span className={'for-sale'}>
      <Icon>add_shopping_cart</Icon>
    </span>
    <span className={'price'}>
      { new Intl.NumberFormat('en-US', { style: 'currency', currency: amount_currency }).format(amount_cents / 100) }
    </span>
  </Button>)
}
