...
class Profile extends Component {
  handleNewSection(lastSection) {
    return (e) => {
      e.preventDefault();
      this.props.createProfileSection({
        variables: {
  ...
  renderProfileSections() {
    ...
    for (let id of groupOrder) {
      ...
      render.push(<div id={'s:' + id} key={id} className='profile-scrollspy'>{renderedSections}</div>);
...
const CreateSectionMutation = (props) => (
  <Mutation mutation={createProfileSection}>
    { (createProfileSection) => <Profile {...props} createProfileSection={createProfileSection} /> }
  </Mutation>
)
