...
class ImageTags extends Component {
  ...
  renderTag(tag, i) {
    ...
    return <li key={i} className={c('tag', { left, right: !left, top, bottom: !top })}
               style={{
      ...
      <Link to={'/'}>
        <img className={'avatar circle'} src={'/administrator/test.png'} alt={ tag.character.name }/>
  ...
  render() {
    ...
    return <ul className={'image-tags'}>
