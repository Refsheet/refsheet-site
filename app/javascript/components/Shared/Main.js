...
//    document.body.removeClass @props.bodyClassName
  ...
  render() {
    ...
    return <main style={ style } id={ this.props.id } className={ classNames.join(' ') }>
      { this.props.children }
    </main>;
  }
}
...
const Main = styled(_Main)`
  ...
  &.main-flex {
    &.split-bg-right {
      background: linear-gradient(to right, ${props => props.theme.background} 50%, ${props => props.theme.cardBackground} 50%);
    }
    
    &.split-bg-left {
      background: linear-gradient(to left, ${props => props.theme.background} 50%, ${props => props.theme.cardBackground} 50%)
    }
