...
this.DropzoneContainer = React.createClass({
        ...
        init() {
          ...
          this.on('error', function(_, error) {
            if (error.errors != null ? error.errors.image : undefined) {
              return Materialize.toast({ html: `Image ${error.errors.image}`, displayLength: 3000, classes: 'red' });
            } else {
              console.error(JSON.stringify(error));
              return Materialize.toast({ html: `An unknown error has occurred :( Please find Mau and tell them this: ${JSON.stringify(error)}` });
            }
          });

          return this.on('success', function(_, data) {
            Materialize.toast({ html: "Image uploaded!", displayLength: 3000, classes: 'green' });
            if (___this.props.onUpload != null) { return ___this.props.onUpload(data); }
          });
        }
      });
    }
  ...
  render() {
    ...
    return <div className={ className } ref='dropzone'>
