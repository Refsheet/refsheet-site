...
this.Tabs = React.createClass({
  ...
  componentDidMount() {
    ...
    return $(document).on('materialize:modal:ready', () => $(window).trigger('resize'));
},
  ...
  render() {
    ...
    const tabs = React.Children.map(this.props.children, child => {
      if ((child != null ? child.type : undefined) === Tab) {
        const liClasses = ['tab'];
        if (!child.props.name) { liClasses.push('no-name'); }
        ...
        if (typeof (child != null ? child.type : undefined) !== 'undefined') { return console.warn(`Children to Tabs should be a Tab, got ${(child != null ? child.type : undefined)}.`); }
    }
    });
