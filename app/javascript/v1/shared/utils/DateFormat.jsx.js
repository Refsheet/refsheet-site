...
this.DateFormat = React.createClass({
  ...
  componentWillUnmount() {
    if (this.state.timer) { return clearTimeout(this.state.timer); }
  },
  ...
  _initialize(props) {
    ...
    if (props.fuzzy) {
      const t = setTimeout(this._fuzzyPoll, (60 - this.date().getSeconds()) * 1000);
  ...
  _p(i, word) {
    if (this.props.short && (word.substring(0,2) === 'mo')) { return `${i}mo`; }
    if (this.props.short) { return `${i}${word.substring(0,1)}`; }
  ...
  date() {
    ...
    if (ts < Math.pow(10, 11)) { ts *= 1000; }
    return new Date(ts);
  },

  fullTime() {
    const date = this.date();
    return `${date.getFullYear()}-${this._z(date.getMonth() + 1)}-${this._z(date.getDate())} ${this._z(date.getHours())}:${this._z(date.getMinutes())}:${this._z(date.getSeconds())}`;
  },

  showDate() {
    const date = this.date();
    let month = this.monthNames[date.getMonth()];
    if (this.props.short) { month = month.substr(0,3); }
    ...
    if (!this.props.dateOnly) { end += ` ${this._z(hour)}:${this._z(date.getMinutes())} ${apm}`; }
    return end;
  },
  ...
  fuzzyDate() {
    ...
    switch (false) {
      case !(elapsed < msPerMinute):
        if (this.props.short) { return '<1m'; } else { return 'recently'; }
      ...
      case !(elapsed < msPerYear):
        return `${about}${this._p(Math.round(elapsed / msPerMonth), 'month')}${ago}`;
      default:
        return `${about}${this._p(Math.round(elapsed / msPerYear), 'year')}${ago}`;
    }
  },
  ...
  render() {
    ...
    return <span className={ this.props.className } title={ this.fullTime() }>
