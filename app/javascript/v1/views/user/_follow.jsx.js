...
this.Views.User.Follow = React.createClass({
  ...
  _handleFollowClick(e) {
    ...
    Model.request(action, '/users/' + this.props.username + '/follow.json', {}, user => {
      if (this.props.onFollow) { return this.props.onFollow(user.followed, this.props.username, user); }
    });
  ...
  render() {
    if (this.context.currentUser && (this.props.username !== this.context.currentUser.username)) {
      const followColor = this.props.followed ? '#ffca28' : 'rgba(255, 255, 255, 0.7)';

      return <a href='#' className='secondary-content btn btn-flat right cs--secondary-color' style={{ border: '1px solid rgba(255,255,255,0.1)' }} onClick={ this._handleFollowClick }>
          { !this.props.short &&
